...
export function Spinner({
  ...
  return (
    <div
      ...
      className={cn(
        'fds-spinner',
        `fds-spinner--color-${color.toLowerCase()}`,
        `fds-spinner--size-${size.toLowerCase()}`,
        label && `fds-spinner--label-${labelPosition.toLowerCase()}`,
        className,
      )}
      {...props}
    >
      <div className="fds-spinner__indicator">
        <svg
          ...
          aria-hidden="true"
        >
          {/* Track — full circle at 9% opacity */}
          <path
      ...
      {label && (
        <span className="fds-spinner__label BodySmallRegular">{label}</span>
      )}
    </div>
  );
}
