 ...
 */
export const CHART_TITLE_TYPOGRAPHY: Record<ChartTitleSize, string> = {
...
/* ═══════════════════════════════════════════════════════════════════════════
   Chart — base wrapper for all chart types
   Figma: 2742:4943
   ═══════════════════════════════════════════════════════════════════════════ */
...
export const Chart = forwardRef<HTMLDivElement, ChartProps>(
  (
    ...
    const hasHeaderRow = title != null || breadcrumb != null;
    const hasHeaderMain = hasHeaderRow || duration != null;
    const hasHeader = hasHeaderMain || actions != null;

    return (
      <div ref={ref} className={cn('fds-chart', className)} {...props}>
        {hasHeader && (
          <div className="fds-chart__header">
            {hasHeaderMain && (
              <div className="fds-chart__header-main">
                {hasHeaderRow && (
                  <div className="fds-chart__header-row">
                    {titleIsString ? (
                      <button
                        type="button"
                        className={cn('fds-chart__title', titleClassName)}
                        onClick={onTitleClick}
                      >
                        <span
                          className={cn(
                            'fds-chart__title-label',
                            CHART_TITLE_TYPOGRAPHY[titleSize],
                          )}
                        >
                          {title}
                        </span>
                ...
                {duration != null && (
                  <div className="fds-chart__duration">
                    <Clock className="fds-chart__duration-icon" aria-hidden="true" />
                    <span className="fds-chart__duration-label BodyMediumRegular">
                      {duration}
                    </span>
                  </div>
                )}
            ...
            {actions != null && <div className="fds-chart__actions">{actions}</div>}
          </div>
        )}

        {filters != null && <div className="fds-chart__filters">{filters}</div>}

        <div className="fds-chart__canvas">{children}</div>
      </div>
    );
  },
);
